...
const StatCard = ({ icon: Icon, title, value, change, href }) => {
  ...
  const wrapperClass = 'overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm transition-all duration-200 hover:border-orange-200 dark:hover:border-orange-800';
  if (href) {
    return (
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        {/* Bookings trend */}
        <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm lg:col-span-2">
          <CardHeader className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 px-4 sm:px-6 py-3 sm:py-4">
            <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">{t('dashboard.weeklyBookings')}</CardTitle>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{t('dashboard.lastSixMonths') || 'آخر 6 أشهر'}</p>
          </CardHeader>
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={bookingsChartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Bar dataKey="bookings" fill={CHART_PRIMARY} radius={[4, 4, 0, 0]} name={t('dashboard.bookings')} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
          <CardHeader className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 px-4 sm:px-6 py-3 sm:py-4">
            <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">{t('dashboard.recentActivity')}</CardTitle>
          </CardHeader>
          <CardContent className="p-4 sm:p-6">
                    ...
                    <div className="flex items-center gap-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 px-3 py-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-gray-900 dark:text-white">
                          {t('dashboard.newBookingFrom')} {booking.student?.firstName || booking.student?.email || t('dashboard.students')}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(booking.createdAt).toLocaleString()}</p>
                      </div>
                    </div>
                  </li>
                ))
      ...
      <section>
        <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
          <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 px-4 sm:px-6 py-3 sm:py-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('dashboard.quickStats')}</h2>
          </div>
