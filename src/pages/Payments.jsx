...
const Payments = () => {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const params = { page, limit: 20, ...(statusFilter && { status: statusFilter }), ...(typeFilter && { type: typeFilter }) };
  ...
  const displayStatus = (status) => (status === 'COMPLETED' ? 'SUCCEEDED' : status);

  const getStatusBadge = (status) => {
    const s = displayStatus(status);
    const badges = {
      SUCCEEDED: 'bg-emerald-100 text-emerald-800 ring-1 ring-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-200',
    ...
    return badges[s] || 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200';
  };

  const getRelatedLabel = (payment) => {
    if (payment.bookingId && payment.booking) {
      return language === 'ar' ? `حجز #${payment.bookingId.slice(0, 8)}` : `Booking #${payment.bookingId.slice(0, 8)}`;
    }
    if (payment.subscriptionId && payment.subscription) {
      const name = payment.subscription?.package?.name || payment.subscriptionId.slice(0, 8);
      return language === 'ar' ? `اشتراك: ${name}` : `Subscription: ${name}`;
    }
    if (payment.courseId && payment.course) {
      return language === 'ar' ? `دورة: ${payment.course.title}` : `Course: ${payment.course.title}`;
    }
    return payment.paymentType || '—';
  };

  const getUserDisplay = (payment) => {
    if (payment.user) {
      const name = [payment.user.firstName, payment.user.lastName].filter(Boolean).join(' ').trim();
      return name || payment.user.email || payment.userId || '—';
    }
    if (payment.booking?.student) {
      const s = payment.booking.student;
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <>
            <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
              <div className="p-5">
                ...
                <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">{formatCurrency(stats.totalRevenue || 0)}</p>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:gap-3">
          ...
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="h-10 min-w-[170px] rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-4 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500">
            <option value="">{language === 'ar' ? 'كل الأنواع' : 'All types'}</option>
            <option value="BOOKING">{language === 'ar' ? 'حجز' : 'Booking'}</option>
            <option value="SUBSCRIPTION">{language === 'ar' ? 'اشتراك' : 'Subscription'}</option>
              ...
              <table className="w-full caption-bottom text-sm">
                <thead className="border-b border-gray-200 bg-gray-100 dark:border-gray-700 dark:bg-gray-700/70">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{language === 'ar' ? 'مرجع فوري' : 'Fawry ref'}</th>
                    ...
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{language === 'ar' ? 'الرابط' : 'Related'}</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {payments.map((payment) => (
                    <tr key={payment.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="px-6 py-4"><div className="text-sm font-mono text-gray-900 dark:text-white">{payment.merchantRefNum || payment.stripePaymentIntentId || payment.id?.slice(0, 8)}</div></td>
                      <td className="px-6 py-4"><div className="text-sm font-mono text-gray-600 dark:text-gray-300">{payment.fawryRefNumber || '—'}</div></td>
                      <td className="px-6 py-4"><div className="text-sm text-gray-900 dark:text-white">{getUserDisplay(payment)}</div></td>
                      <td className="px-6 py-4"><div className="text-sm text-gray-700 dark:text-gray-300">{getRelatedLabel(payment)}</div></td>
                      <td className="px-6 py-4"><div className="text-sm font-semibold text-gray-900 dark:text-white">{formatCurrency(payment.amount)}</div></td>
                      <td className="px-6 py-4">
                        <span className={cn('px-2.5 py-1 rounded-full text-xs font-semibold', getStatusBadge(payment.status))}>{displayStatus(payment.status)}</span>
                      </td>
                      <td className="px-6 py-4">
